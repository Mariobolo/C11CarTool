...
> v0.3.6 更新「数据全量对齐」：状态条「📊 信号」进入全车信号清单页，解析器能拿到的信号全部可见；同一数据若有多个来源（settings / eventId / XML 节点 / TPMS），各占一行并在末尾标注渠道，互不覆盖。新增行程里程·时间·平均能耗（EnergyDataBinder）、驾驶模式、制动踏板开度、天窗/遮阳帘、车窗开度、灯光状态、屏幕亮度、充电状态、GPS、座椅通风等；仪表盘抓取 TAG 由 5 个补齐到 12 个。制动踏板经日志实测为连续开度值（%），已纠正早期规范的 0/1/2 枚举猜测。
...
- **全车信号清单**：状态条「📊 信号」切换整页，按动力底盘 / 空调 / 车身 / 灯光 / 系统能耗 / 充电 / 胎压 / 行程 / 定位分组，逐行列出「名称 = 值 [渠道]」，多渠道值全部保留
...
| logcat 被动解析 | 车速 / 档位 / 电压电流 / 电量 / 车外温度 / TPMS / 车门 / 行程三件套 / 灯光状态等 | 只读，用于仪表盘与信号清单展示 |
