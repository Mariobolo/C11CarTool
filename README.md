...
> v0.3.5.1 更新：仪表盘状态条显示版本号（防装旧包）；Web 弹窗内置二维码，手机连同一 WiFi 扫码即开；一键诊断新增「本地 ADB 公钥指纹」，与车机授权弹窗指纹比对即可定位反复弹授权的归属；车控实验清单按 0921 真机标定重排（温度/风量/前除霜为 settings 通道，已验证项标 ✅，锁车/车窗单列 ⚠ 实验性）。
...
- **Web 遥控**：App 内置 HTTP 服务，随启动自启；状态条与 Web 卡片弹窗显示访问地址和二维码，同一局域网（wlan0）下手机扫码即可车控与查看日志（默认 8080，端口冲突自动切换）
- **工程模式**：一键诊断（端口监听、权限、avc denied、通道自测、本地 ADB 公钥指纹）、一键全测、车控实验矩阵、日志导出（仪表盘右上角齿轮进入）
